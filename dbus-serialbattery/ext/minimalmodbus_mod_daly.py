...
#   MODIFIED VERSION OF MINIMALMODBUS 2.1.1, FOR DALY BMS DEVICES
#   Accepts slave_send_address_offset, which is only added to the
#   slave address while sending requests. Responses are still
#   processed from the configured slaveaddress.
...
class Instrument:
    ...
    def __init__(
        ...
        slaveaddress: int,
        slave_send_address_offset: int = 0x00,
        mode: str = MODE_RTU,
        ...
        """
        if (self.address + slave_send_address_offset) >= 0 and (self.address + slave_send_address_offset) <= 247:
            self.slave_send_address_offset = slave_send_address_offset
        else:
            raise MasterReportedException("slaveaddress with slave_send_address_offset added is out of bounds [0:247]")
        """Slave address offset (int). Most often set by the constructor (see the class
        documentation).

        Daly HKMS and 100balance BMS reply to Modbus requests with their slave address,
        but the requests have to be sent from an offset address. This setting applies
        this offset to the slave address while sending, but still receives the response
        from the real slave address.
        """        
    ...
    def _perform_command(self, functioncode: int, payload_to_slave: bytes) -> bytes:
        ...
        request_bytes = _embed_payload(
            self.address+self.slave_send_address_offset, self.mode, functioncode, payload_to_slave
        )
